...
from raven.blueprints.raven_socketio import administrator_only
...
class PluginManager:
    ...
    def _add_plugin(self, plugin: Plugin) -> None:
        if not isinstance(plugin, Plugin):
            raise ValueError("The Raven plugin must be an instance of Plugin")
    ...
    def install_plugin(json: dict):
        ...
        if json.get("action") != "install_local":
            for plugin_prefix in app.config.get("RAVEN_PLUGIN_PREFIXES"):
        ...
        if json.get("action") == "delete":
            ...
            command = f"{sys.executable} -m pip --no-input install {json.get('plugin_distro')} -i {app.config.get('RAVEN_PLUGIN_REPO')}"
