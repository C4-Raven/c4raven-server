...
class CoTController:
    ...
    def run(self):
        ...
        self.rabbit_channel.basic_qos(
            prefetch_count=self.context.app.config.get("RAVEN_RABBITMQ_PREFETCH")
        )
    ...
    def parse_point(self, event, uid, cot_id):
        ...
        if point and not point.attrs["lat"].startswith("999"):
            ...
            with self.context:
                ...
                if self.context.app.config.get("RAVEN_ENABLE_MESHTASTIC"):
                    try:
                        ...
                        if eud.last_meshtastic_publish is None or (
                            ...
                            "RAVEN_MESHTASTIC_PUBLISH_INTERVAL"
                        ):
                            ...
                            try:
                                ...
                                if eud.platform != "Meshtastic":
                                    ...
                                    if (
                                        ...
                                        "RAVEN_MESHTASTIC_NODEINFO_INTERVAL"
    ...
    def get_protobuf(
        ...
        service_envelope.gateway_id = "Raven"
    ...
    def publish_to_meshtastic(self, body):
        for channel in self.get_meshtastic_channels():
            ...
            routing_key = "{}.2.e.{}.outgoing".format(
                self.context.app.config.get("RAVEN_MESHTASTIC_TOPIC"), channel
            )
    ...
    def route_cot(self, event, uid: str, user_id: int, target_groups: list[str] | None = None):
        ...
        # Federated-in traffic (see federation_hub_daemon.py's
        # _handle_federated_event) carries the sending server's own group
        # name(s) explicitly, so route straight to the same-named group(s)
        # here instead of falling back to user_id's own memberships or
        # __ANON__ -- this is what lets a federated user actually receive
        # whatever group they were assigned to on the *other* server,
        # rather than only ever seeing traffic for whichever group the
        # shared "Server" attribution account itself happens to belong to.
        if not destinations and target_groups:
    ...
    def on_message(
        ...
        try:
            ...
            if uid == self.context.app.config["RAVEN_NODE_ID"]:
                uid = None
...
def setup_logging(app):
    ...
    fh = TimedRotatingFileHandler(
        os.path.join(app.config.get("RAVEN_DATA_FOLDER"), "logs", "cot_parser.log"),
