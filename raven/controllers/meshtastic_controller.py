...
from raven.extensions import db, logger, socketio
...
class MeshtasticController(RabbitMQClient):
    ...
    def get_channels(self):
        with self.context:
            ...
            self.context.app.config.update({"RAVEN_MESHTASTIC_DOWNLINK_CHANNELS": downlink_channels})
    ...
    def decrypt(self, mp: mesh_pb2.MeshPacket, channel_name: str) -> mesh_pb2.MeshPacket:
        if channel_name not in self.context.app.config.get("RAVEN_MESHTASTIC_DOWNLINK_CHANNELS"):
            logger.error(
                f"{channel_name} not in {self.context.app.config.get('RAVEN_MESHTASTIC_DOWNLINK_CHANNELS')}"
            )
            return mp
        ...
        psk = self.context.app.config.get("RAVEN_MESHTASTIC_DOWNLINK_CHANNELS")[channel_name]
    ...
    def on_message(
        ...
        for channel in self.context.app.config.get("RAVEN_MESHTASTIC_DOWNLINK_CHANNELS"):
                ...
                if portnum == "TEXT_MESSAGE_APP":
                    try:
                        if to_id == "all":
                            self.rabbit_channel.basic_publish(
                                exchange="groups",
                                routing_key=f"{self.context.app.config.get('RAVEN_MESHTASTIC_GROUP')}.OUT",
                                body=message,
                                properties=pika.BasicProperties(
                                    expiration=self.context.app.config.get("RAVEN_RABBITMQ_TTL")
                                ),
