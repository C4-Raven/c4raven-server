    ...
    def __init__(self, context: Flask):
        ...
        try:
            rabbit_credentials = pika.PlainCredentials(
                self.context.app.config.get("RAVEN_RABBITMQ_USERNAME"),
                self.context.app.config.get("RAVEN_RABBITMQ_PASSWORD"),
            )
            rabbit_host = self.context.app.config.get("RAVEN_RABBITMQ_SERVER_ADDRESS")
