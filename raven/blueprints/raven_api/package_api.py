...
from raven.blueprints.marti_api.certificate_enrollment_api import basic_auth
...
from raven.forms.package_form import PackageForm, PackageUpdateForm
from raven.models.Packages import Packages
...
def download_package(package_name, atak_version=None):
    ...
    return send_from_directory(
        os.path.join(app.config.get("RAVEN_DATA_FOLDER"), "packages"), secure_filename(package_name)
    )
...
def head_product_infz():
    ...
    if os.path.exists(os.path.join(app.config.get("RAVEN_DATA_FOLDER"), "packages", "product.infz")):
...
def head_product_infz_with_version(atak_version: str):
    ...
    if os.path.exists(
        os.path.join(app.config.get("RAVEN_DATA_FOLDER"), "packages", atak_version, "product.infz")
    ):
        return jsonify({"success": True})
...
def get_product_infz():
    ...
    if not cert:
        return "", 401
    return send_from_directory(
        os.path.join(app.config.get("RAVEN_DATA_FOLDER"), "packages"), "product.infz"
    )
...
def create_product_infz(atak_version: str | None):
    product_infz_file = os.path.join(app.config.get("RAVEN_DATA_FOLDER"), "packages", "product.infz")
    product_inf_file = os.path.join(app.config.get("RAVEN_DATA_FOLDER"), "packages", "product.inf")

    if atak_version:
        atak_version = bleach.clean(atak_version)
        product_infz_file = os.path.join(
            app.config.get("RAVEN_DATA_FOLDER"), "packages", atak_version, "product.infz"
        )
        product_inf_file = os.path.join(
            app.config.get("RAVEN_DATA_FOLDER"), "packages", atak_version, "product.inf"
        )
        os.makedirs(
            os.path.join(app.config.get("RAVEN_DATA_FOLDER"), "packages", atak_version), exist_ok=True
        )
...
def add_package():
    ...
    os.makedirs(os.path.join(app.config.get("RAVEN_DATA_FOLDER"), "packages"), exist_ok=True)

    apk_filename = secure_filename(request.files["apk"].filename)

    request.files["apk"].save(
        os.path.join(app.config.get("RAVEN_DATA_FOLDER"), "packages", apk_filename)
    )
    ...
    if existing_package:
        ...
        if os.path.exists(
            os.path.join(app.config.get("RAVEN_DATA_FOLDER"), "packages", existing_package.file_name)
        ):
            os.remove(
                os.path.join(
                    app.config.get("RAVEN_DATA_FOLDER"), "packages", existing_package.file_name
                )
            )
...
def delete_package():
    ...
    package = package[0]
    if os.path.exists(
        os.path.join(app.config.get("RAVEN_DATA_FOLDER"), "packages", package.file_name)
    ):
        os.remove(os.path.join(app.config.get("RAVEN_DATA_FOLDER"), "packages", package.file_name))
