...
from raven.extensions import babel, db, ldap_manager, logger
from raven.models.Alert import Alert
from raven.models.APSchedulerJobs import APSchedulerJobs
...
def change_config_setting(setting, value):
    try:
        with open(
            os.path.join(app.config.get("RAVEN_DATA_FOLDER"), "config.yml"), "r"
        ) as config_file:
            config = yaml.safe_load(config_file.read())

        config[setting] = value
        with open(
            os.path.join(app.config.get("RAVEN_DATA_FOLDER"), "config.yml"), "w"
        ) as config_file:
...
def route_cot(event: str, user: User):
    ...
    if not group_memberships:
        ...
        channel.basic_publish(
            ...
            body=json.dumps({"uid": app.config["RAVEN_NODE_ID"], "cot": str(event)}),
        ...
        try:
            ...
            for filename in filenames:
                file_hash = hashlib.sha256(
                    open(
                        os.path.join(app.config.get("RAVEN_CA_FOLDER"), "certs", username, filename),
                        "rb",
                    ).read()
                ).hexdigest()

                # A user can (and, via the Users page's Join QR Code button,
                # is expected to) have this regenerated more than once --
                # DataPackage.filename is unique, so a second call for the
                # same user hit that constraint and failed outright instead
                # of just pointing the existing row at the freshly-issued
                # cert. Update in place instead, the same way add_citrap
                # already handles its own equivalent collision.
                existing_data_package = db.session.execute(
                    db.session.query(DataPackage).filter_by(filename=filename)
                ).first()

                if existing_data_package:
                    data_package = existing_data_package[0]
                else:
                        ...
                        jsonify(
                            {
                                "success": False,
                                "error": gettext(
                                    "Failed to save certificate for %(username)s", username=username
                                ),
                            }
                        ),
                        400,
                ...
                copyfile(
                    os.path.join(
                        app.config.get("RAVEN_CA_FOLDER"), "certs", username, "{}".format(filename)
                    ),
                ...
                cert.expiration_date = datetime.datetime.today() + datetime.timedelta(
                    days=app.config.get("RAVEN_CA_EXPIRATION_TIME")
                )
                cert.server_address = urlparse(request.url_root).hostname
                cert.server_port = app.config.get("RAVEN_SSL_STREAMING_PORT")
                cert.truststore_filename = truststore_filename
                cert.user_cert_filename = user_filename
                cert.cert_password = app.config.get("RAVEN_CA_PASSWORD")
...
def rabbitmq_auth(path):
    ...
    # Only allow requests to this route from the RabbitMQ server. remote_addr
    # alone isn't enough: nginx's own proxied connections to this app also
    # come from 127.0.0.1 (same box as RabbitMQ), so a request routed through
    # nginx's public /api passthrough looks identical to RabbitMQ's own call
    # by that check alone -- this endpoint would otherwise let anyone on the
    # internet brute-force any account's password (and learn who's an admin)
    # via this route, completely unauthenticated. nginx always sets
    # X-Forwarded-For on every request it proxies, which a genuinely direct
    # call from RabbitMQ never has, so its presence reliably means "this
    # came through nginx from the outside" regardless of what remote_addr says.
    if request.headers.get("X-Forwarded-For") or request.remote_addr != app.config.get(
        "RAVEN_RABBITMQ_SERVER_ADDRESS"
    ):
        return "deny", 200
    ...
    if app.config.get("RAVEN_ENABLE_LDAP"):
        ...
        if result.status == AuthenticationResponseStatus.success:
            ...
            for group in result.user_groups:
                if group["cn"] == app.config.get("RAVEN_LDAP_ADMIN_GROUP"):
...
def get_settings():
    ...
    ``Raven_SERVER-ADDRESS,SERVER-ADDRESS,8089,SSL``
    """
    url = urlparse(request.url_root).hostname
    return "Raven_{},{},{},SSL".format(url, url, app.config.get("RAVEN_SSL_STREAMING_PORT"))
