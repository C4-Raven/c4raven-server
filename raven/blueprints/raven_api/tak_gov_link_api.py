...
from raven.blueprints.raven_api.api import change_config_setting
from raven.blueprints.raven_api.package_api import create_product_infz
...
def link_tak_gov_account():
    """Interacts with tak.gov's eud_api to get the user_code and device_code required to link Raven to a tak.gov account
...
def get_initial_tak_gov_token():
    ...
    try:
        ...
        change_config_setting("RAVEN_TAK_GOV_LINKED", True)
        change_config_setting("RAVEN_TAK_GOV_REFRESH_TOKEN", refresh_token)

        app.config.update({"RAVEN_TAK_GOV_LINKED": True})
        app.config.update({"RAVEN_TAK_GOV_REFRESH_TOKEN": refresh_token})
...
def get_new_tak_gov_token():
    ...
    if not app.config.get("RAVEN_TAK_GOV_LINKED") or not app.config.get("RAVEN_TAK_GOV_REFRESH_TOKEN"):
...
def unlink_tak_gov_account():
    if not app.config.get("RAVEN_TAK_GOV_LINKED"):
        return jsonify({"success": False, "error": "Not linked to a TAK.gov account"}), 400

    change_config_setting("RAVEN_TAK_GOV_LINKED", False)
    change_config_setting("RAVEN_TAK_GOV_REFRESH_TOKEN", None)

    app.config.update({"RAVEN_TAK_GOV_LINKED": False})
    app.config.update({"RAVEN_TAK_GOV_REFRESH_TOKEN": None})
...
def check_if_linked():
    return jsonify(
        {"success": True, "tak_gov_account_linked": app.config.get("RAVEN_TAK_GOV_LINKED")}
    )
...
def download_plugin():
    ...
    with open(os.path.join(app.config.get("RAVEN_DATA_FOLDER"), "packages", filename), "wb") as f:
