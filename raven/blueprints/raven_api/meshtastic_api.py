...
def save_channel(channel_set: apponly_pb2.ChannelSet):
    for channel_settings in channel_set.settings:
        ...
        if channel_settings.downlink_enabled and channel_settings.name not in app.config.get(
            ...
            app.config.get("RAVEN_MESHTASTIC_DOWNLINK_CHANNELS")[
                channel_settings.name
...
def delete_channel():
    ...
    try:
        ...
        if channel.name in app.config.get("RAVEN_MESHTASTIC_DOWNLINK_CHANNELS"):
            app.config.get("RAVEN_MESHTASTIC_DOWNLINK_CHANNELS").pop(channel.name)
