    ...
    return jsonify(
        {
            ...
            "data": app.config.get("RAVEN_ENABLE_CHANNELS"),
        }
    )
...
def get_all_groups():
    ...
    if not app.config.get("RAVEN_ENABLE_LDAP"):
        ...
        for group in groups:
            if app.config.get("RAVEN_LDAP_GROUP_PREFIX") and not group["cn"].startswith(
                app.config.get("RAVEN_LDAP_GROUP_PREFIX")
            ):
                continue
...
def get_ldap_groups():
    ...
    if not group_name.startswith(app.config.get("RAVEN_LDAP_GROUP_PREFIX")):
        return (
            jsonify(
                {
                    "success": False,
                    "error": gettext(
                        ...
                        prefix=app.config.get("RAVEN_LDAP_GROUP_PREFIX"),
                    ),
                }
            ),
            400,
...
def get_ldap_group_prefix():
    ...
    if app.config.get("RAVEN_ENABLE_LDAP"):
        response["data"] = app.config.get("RAVEN_LDAP_GROUP_PREFIX")
