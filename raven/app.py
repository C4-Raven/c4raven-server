    ...
    with app.app_context():
        if not app.config.get("RAVEN_ENABLE_LDAP"):
            ...
            adsb_group = db.session.execute(
                db.session.query(Group).filter_by(name=app.config.get("RAVEN_ADSB_GROUP"))
            ).first()
            ais_group = db.session.execute(
                db.session.query(Group).filter_by(name=app.config.get("RAVEN_AIS_GROUP"))
            ).first()
            meshtastic_group = db.session.execute(
                db.session.query(Group).filter_by(name=app.config.get("RAVEN_MESHTASTIC_GROUP"))
            ).first()
            ...
            if not adsb_group:
                logger.info(f"Creating the {app.config.get('RAVEN_ADSB_GROUP')} group")
                adsb_group = Group()
                adsb_group.name = app.config.get("RAVEN_ADSB_GROUP")
            ...
            if not ais_group:
                logger.info(f"Creating the {app.config.get('RAVEN_AIS_GROUP')} group")
                ais_group = Group()
                ais_group.name = app.config.get("RAVEN_AIS_GROUP")
            ...
            if not meshtastic_group:
                logger.info(f"Creating the {app.config.get('RAVEN_MESHTASTIC_GROUP')} group")
                meshtastic_group = Group()
                meshtastic_group.name = app.config.get("RAVEN_MESHTASTIC_GROUP")
