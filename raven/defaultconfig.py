...
class DefaultConfig:
    ...
    # Raven user account that inbound federated CoT is attributed to when
    # it's fed into cot_parser -- must be a member of RAVEN_FEDHUB_FEDERATE_GROUP
    # (Admin -> Groups -> Fed group) or federated contacts won't route
    # anywhere and won't show up on the map. Reuses the existing protected
    # "Server" system account rather than a dedicated one.
    RAVEN_FEDHUB_FEDERATE_USERNAME = os.getenv("RAVEN_FEDHUB_FEDERATE_USERNAME", "Server")
    ...
    JOBS = [
        ...
        {
            "id": "reconcile_eud_status",
            "func": "raven.blueprints.scheduled_jobs:reconcile_eud_status",
            "trigger": "interval",
            "seconds": 0,
            "minutes": 1,
            "next_run_time": None,
